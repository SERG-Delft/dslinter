...
class RandomnessControlDataloaderPytorchChecker(BaseChecker):
    ...
    def visit_import_from(self, import_from_node: astroid.ImportFrom):
        """
        Check whether there is DataLoader imported.
        """
        if(
            hasattr(import_from_node, "modname")
            and import_from_node.modname == "torch.utils.data"
            and hasattr(import_from_node, "names")
        ):
            for name, _ in import_from_node.names:
                if name == "DataLoader":
